...
bool Filesum::Initialize(int alg, uint32_t width)
{
	switch (alg) {
	case kFilesumMD5:
		sum = new MD5Hashsum();
...
void Filesum::Update(const char * buf, size_t bufsize)
{
	sum->Update((const unsigned char*)buf, bufsize);
}

void Filesum::Final(bool ucase)
{
	sum->Final(ucase, hex);
}
